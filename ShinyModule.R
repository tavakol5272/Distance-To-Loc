  ...
  fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(width = 4,
                   ...
                   fluidRow(
                     column(6, numericInput(ns("posi_lon"), "Longitude of Reference Location:", value=0, min = -180, max = 180,step=0.00001)),
                     column(6, numericInput(ns("posi_lat"), "Latitude of Reference Location:", value=0, min = -90, max = 90,step=0.00001))  ),
                   ...
                   h4("Download:"),
                   downloadButton(ns("save_plot"), "Download plot", class = "btn-sm")
      ),
...
shinyModule <- function(input, output, session, data) {
  ...
  mv_sel <- reactive({
    req(input$animals)
    mv[mv[[trk_col]] %in% input$animals, ]
  })
